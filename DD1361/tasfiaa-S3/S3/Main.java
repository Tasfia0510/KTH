...
public class Main {
	public static void main(String[] args) {
		...
		DFA automaton = new DFA(IO.getInt(), IO.getInt());	// stateCount, startState
		int accept = IO.getInt();
		for (int i = 0; i < accept; ++i)
			automaton.setAccepting(IO.getInt());			// accepting states
		...
		for (int i = 0; i < transitions; ++i) {
			...
			for (char x = lo; x <= hi; ++x)
				automaton.addTransition(from, to, x);	 // transitions
		}
		int bound = IO.getInt();
